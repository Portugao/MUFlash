 ...
 *
 * This is the base repository class for movie entities.
 */
class MUFlash_Entity_Repository_Base_Movie extends EntityRepository
{
    /**
    ...
    public function truncateTable()
    {
        $query = $this->getEntityManager()
                 ->createQuery('DELETE MUFlash_Entity_Movie');
        $query->execute();
    }
     ...
     *
     * @return array|MUFlash_Entity_Movie retrieved data array or MUFlash_Entity_Movie instance
     */
     ...
     * @param boolean $useJoins Whether to include joining related objects (optional) (default=true).
     *
     * @return ArrayCollection collection containing retrieved MUFlash_Entity_Movie instances
     */
    ...
    public function selectSearch($fragment = '', $exclude = array(), $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true)
    {
        ...
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.preview = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.flashFile = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.backgroundColour LIKE \'%' . $fragment . '%\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.width LIKE \'%' . $fragment . '%\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.height LIKE \'%' . $fragment . '%\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.playerVersion LIKE \'%' . $fragment . '%\'';
    ...
    public function selectCount($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numMovies';
        if ($useJoins === true) {
     ...
     * @param int    $excludeid  Id of movies to exclude (optional).
...
// TODO - see https://github.com/zikula/core/issues/118
        ...
        //$fu = new FilterUtil('MUFlash', $this);
